...
- 📝 **第四列编辑器** — 对话右侧 CodeMirror 编辑面板，双击文件打开
- 🎯 **选中发送** — MD 选中文本 / HTML 选中 DOM，填入建议后直接发给对话
- 🔍 **命令面板** — Ctrl+K 快速搜索，40+ 命令，拼音匹配
- 📁 **文件面板** — 文件树浏览、代码预览、Diff 对比、右键菜单
- 💬 **会话管理** — 智能复用（跳过重复）、创建/删除/重命名/续接，SQLite 持久化
- ⚙️ **设置面板** — API Key / Base URL / Model 配置 + 连接测试
- 🛡️ **权限控制** — 6 种模式，自动同步 `~/.claude/settings.json`
- 🎨 **亮/暗双主题** — CSS 变量驱动，语义化 BEM class 命名
- 📊 **Office 预览** — DOCX (mammoth)、XLSX/CSV (SheetJS)、PPTX (文本提取)
- 🌐 **i18n** — 中英双语
- 🔔 **桌面通知** — 任务完成时弹出（含耗时和 token 统计）
- ✨ **提示词优化** — 一键 AI 改写模糊描述
- 🔧 **一键安装 CC** — 内置 Claude Code CLI 安装脚本
- 📋 **工作清单** — CC TodoWrite 任务列表，状态标记芯片
...
| 版本 | 日期 | 更新要点 |
|------|------|----------|
| [0.12.0](docs/变更记录.md) | 2026-07-17 | 时间线tooltip优化、右键菜单边界裁剪、MD大纲解析修复 |
| [0.11.0](docs/变更记录.md) | 2026-07-16 | Markdown渲染引擎替换(marked)、时间线分页压缩、ContentBlock合并修复、30个新测试 |
| [0.10.0](docs/变更记录.md) | 2026-07-14 | Excel选区修复、tip统一、选区卡片合并、MD大纲、链接浏览器打开 |
| [0.9.0](docs/变更记录.md) | 2026-07-12 | 弹框样式统一、PPTX 文本提取、xlsx 选区、Office 预览、TodoPanel |
| [0.8.0](docs/变更记录.md) | 2026-07-08 | 四列布局重构、HTML 预览宽度预设、工具块摘要优化、会话缓存修复 |
| [0.7.0](docs/变更记录.md) | 2026-07-08 | Git 状态面板、提交、Diff 第四列、amend 修复 |
| [0.6.0](docs/变更记录.md) | 2026-07-07 | 第四列编辑器、MD/HTML 选中发送、CSS 语义化命名、智能会话复用 |
